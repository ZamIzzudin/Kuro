  ...
  return (
    <div className="mx-auto max-w-[1400px]">
      ...
      {tasksReq.error && (
        <div className="mt-4">
          <ErrorNote>
            {tasksReq.error instanceof Error ? tasksReq.error.message : 'Gagal memuat task'}
          </ErrorNote>
        </div>
      )}
  ...
  return (
    <article className="rounded-md border border-line bg-surface p-4 shadow-soft transition hover:shadow-md">
      <div className="flex items-start justify-between gap-2">
        ...
        {task.assignee && (
          <span className="truncate text-[11px] font-bold text-ink-500">{task.assignee.name}</span>
        )}
      </div>
