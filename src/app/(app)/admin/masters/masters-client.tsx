...
export function MastersClient() {
  ...
  const { data, mutate, isLoading } = useSWR<{ items: MasterItem[] }>(active.endpoint, fetcher)
  ...
  async function toggle(item: MasterItem) {
    setRowError(null)
    try {
      await apiSend(`${active.endpoint}/${item.id}`, 'PATCH', { isActive: !item.isActive })
      await mutate()
    } catch (err) {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-ink-500">Data acuan untuk task & time entry</p>
      </div>
      ...
      <div className="mt-4 overflow-hidden rounded-xl border border-line bg-surface shadow-sm">
        {isLoading ? (
          ...
          <div className="py-14 text-center">
            <p className="text-sm font-bold text-ink-700">Belum ada {active.label.toLowerCase()}</p>
            ...
            {items.map((item) => (
              <li key={item.id} className="flex items-center justify-between gap-3 px-5 py-3.5">
                ...
                <div className="flex shrink-0 items-center gap-2">
                  <Button variant="ghost" size="sm" onClick={() => setRenameFor(item)}>
                    <Pencil className="h-3.5 w-3.5" /> Ubah
                  </Button>
                  <Button variant="secondary" size="sm" onClick={() => toggle(item)}>
...
function RenameModal({
  ...
  async function submit(e: FormEvent) {
    ...
    setError(null)
    try {
      await apiSend(`${endpoint}/${current.id}`, 'PATCH', { name: name.trim() })
      onDone()
      onClose()
    } catch (err) {
    ...
    <Modal open={!!item} onClose={onClose} title="Ubah Nama">
      <form onSubmit={submit} className="space-y-4">
        <div>
          ...
          <Input id="rn-name" required minLength={2} maxLength={100} value={name} onChange={(e) => setName(e.target.value)} />
        </div>
